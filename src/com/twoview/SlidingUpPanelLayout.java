...
public class SlidingUpPanelLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (i == 1) {
				// namh : second view : LinearLayout
                lp.slideable = true;
                lp.dimWhenOffset = true;
                mSlideableView = child;
                mCanSlide = true;
            } else {
            	// namh : first view : WebView
                height -= panelHeight;
            }
	...
	private boolean isDragViewTipHit(int x, int y) {
        View v = mDragView != null ? mDragView : mSlideableView;
        if (v == null) return false;
        ...
        return screenX >= viewLocation[0] && screenX < viewLocation[0] + v.getWidth() &&
                screenY >= viewLocation[1] && screenY < viewLocation[1] + mPanelHeight;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                mDragViewHit = isDragViewTipHit((int) x, (int) y);
                if (mDragViewHit) {
					// namh
					// sliding panel is touched, 
					// so that the event will be dealt with onTouchEvent of here.
                    interceptTap = true;
                }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        mDragHelper.processTouchEvent(ev);	// namh, all the dragging operation is treated here.
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (dx * dx + dy * dy < slop * slop &&
                        isDragViewTipHit((int) x, (int) y)) {
                    // hit the sliding pane, during sliding
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewCaptured(View capturedChild, int activePointerId) {
			...
			// namh
			// this method is invoked right before start to drag
            setAllChildrenVisible();
        }
